...
def training_loop(model, train_loader, val_loader, criterion, optimizer, scheduler, scaler, device, args, early_stopping):
    """Function to handle the training loop."""
    ...
    with open(args.metrics_csv, mode='w', newline='') as csv_file:
        ...
        for epoch in range(1, args.num_epochs + 1):
            avg_train_loss = train_one_epoch(
                ...
                args.mixed_precision in ["fp16", "bf16"], 
                torch.bfloat16 if args.mixed_precision == "bf16" else torch.float16,
                tensorboard_logdir=args.tensorboard_logdir if args.use_tensorboard else None, 
                wandb_enabled=args.use_wandb
            )
            avg_val_loss, val_accuracy = validate(
                model, val_loader, criterion, device,
                tensorboard_logdir=args.tensorboard_logdir if args.use_tensorboard else None, 
                wandb_enabled=args.use_wandb, epoch=epoch
            )
