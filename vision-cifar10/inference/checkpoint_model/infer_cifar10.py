...
def get_model_architecture(model_name, num_classes=10):
    """
    Create a model with the specified architecture and empty weights.
    Configures the output layer for CIFAR-10 dataset (10 classes).
    """
...
def load_cifar10_labels():
    """
    Return the class labels for CIFAR-10.
    """
...
def main():
    parser = argparse.ArgumentParser(description="Run inference with a PyTorch model on an image")
    ...
    # Create model
    try:
        model = get_model_architecture(args.model)
    ...
    # Get top predictions
    class_labels = load_cifar10_labels()
