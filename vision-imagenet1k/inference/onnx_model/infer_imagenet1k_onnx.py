...
def load_labels(labels_path):
    ...
    if not os.path.exists(labels_path):
        print(f"Warning: Labels file {labels_path} not found. Using index numbers instead.")
        return [str(i) for i in range(1000)]  # ImageNet has 1000 classes
...
def run_inference(model_path, image_path, labels_path=None, num_threads=None):
    ...
    if -1 in input_shape or 'batch_size' in str(input_shape):
        input_size = (224, 224)  # Default for most ImageNet models
    else:
    ...
    # Load labels
    labels = load_labels(labels_path) if labels_path else [str(i) for i in range(1000)]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run inference with ImageNet ONNX model on an image")
    parser.add_argument("--model", required=True, help="Path to ONNX model file")
    parser.add_argument("--image", required=True, help="Path to input image")
    parser.add_argument("--labels", help="Path to text file with ImageNet class labels (one per line)")
